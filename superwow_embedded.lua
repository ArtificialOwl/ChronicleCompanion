-- =============================================================================
-- Embedded SuperWoWLogger (based on SuperWowCombatLogger by Shino)
-- =============================================================================
-- This is a bundled version of SuperWowCombatLogger that loads automatically
-- if the user doesn't have the standalone addon installed.
-- =============================================================================

function Chronicle:LoadEmbeddedSuperWoWLogger()
    -- Check if already loaded externally
    if RPLL then
        self:DebugPrint("SuperWoWLogger already loaded externally, skipping embedded version")
        return false
    end
    
    -- Check SuperWoW dependency
    if not SetAutoloot then
        self:DebugPrint("SuperWoW not detected, cannot load embedded SuperWoWLogger")
        return false
    end
    
    self:DebugPrint("Loading embedded SuperWoWLogger")
    
    -- =========================================================================
    -- Create RPLL frame (replaces RPLLCollector.xml)
    -- =========================================================================
    local RPLL = CreateFrame("Frame", "RPLL")
    RPLL.turtlogsCompat = true -- Assume compatible until load is complete
    ...
    -- =========================================================================
    -- BEGIN SUPERWOWLOGGER CODE (from SuperWowCombatLogger/core.lua)
    -- =========================================================================
    ...
    local function LogCastEventV2(caster, target, event, spellID, castDuration)
      ...
      if targetName then
        if rank ~= "" then
          ...
          if RPLL.turtlogsCompat then CombatLogAdd(format(fmt_with_target, casterName, verb, spell, spellID, targetName)) end
          CombatLogAdd(format(fmt_raw_with_target, caster, casterName, verb, spell, spellID, target, targetName), 1)
        end
      else
        if rank ~= "" then
          if RPLL.turtlogsCompat then CombatLogAdd(format(fmt_with_rank, casterName, verb, spell, spellID, rank)) end
          CombatLogAdd(format(fmt_raw_with_rank, caster, casterName, verb, spell, spellID, rank), 1)
        else
          if RPLL.turtlogsCompat then CombatLogAdd(format(fmt_simple, casterName, verb, spell, spellID)) end
    ...
    -- =========================================================================
    -- END SUPERWOWLOGGER CODE
    -- =========================================================================
    
    self.embeddedSuperWoWLogger = true
    self:DebugPrint("Embedded SuperWoWLogger loaded successfully")
    RPLL.turtlogsCompat = Chronicle:GetSetting("turtlogsCompatibility")
    return true
end
